/*═════════════════════════════
 * Adaptado por la BuhardillaDelLoco
 * Mandragora Tools - 2019
 * http://bit.ly/2MUnXcj
 *═════════════════════════════*/
...
bool modoExcep = true; // <-----  Modificar a [false] para un comportamiento normal
...
void delivery (String fileName) {
  ...
  if (dataFile) {
    ...
    Serial.println(F("Error al abrir el script"));
  }
}
...
void management() {
  ...
  Serial.println(F("Playloads disponibles:"));
  printDirectory(root, 0);
  root.close();
  Serial.println();
  Serial.println(F("Modos disponibles: "));
  Serial.println(F("m => Modo de gestión "));
  Serial.println(F("a => Modo de desarmado automático"));
  Serial.println(F("c => Modo de entrega continua"));
  Serial.println();
  Serial.print(F("Modo actual: "));
  Serial.println(mode);
  Serial.print(F("Lenguaje actual: "));
  Serial.println(lang);
...
void setup() {
  ...
  if (!SD.begin(chipSelect)) {
    Serial.println(F("Fallo en tarjeta, o no esta insertada"));
    return;
  }
